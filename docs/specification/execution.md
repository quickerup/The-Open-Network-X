...
3. The cost of reserving it now is low: `crates/onx-state-model`'s `Cell` structure already carries an `is_special` flag (`state-model.md` §4.2's `is_special_flag` bit) with no defined meaning yet. This decision assigns it one, rather than leaving a reserved-but-meaningless bit as a footgun for whoever defines the instruction set later.
