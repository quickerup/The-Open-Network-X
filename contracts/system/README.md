...
- `bytecode/elector.tvm` / `src/elector.rs` — validator stake deposit and election winner logic.
- `bytecode/config.tvm` / `src/config.rs` — parameters for gas rates and validator set config.
- `src/storage.rs` — global workchain configuration root contract source.
