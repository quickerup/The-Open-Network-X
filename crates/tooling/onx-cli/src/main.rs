use clap::Parser;
use onx_cli::{
    balance_query, deploy_contract_boc, serialize_boc_transfer, wallet_create, Cli, Command,
    WalletCommand,
};
use std::process;
    ...
    match cli.command {
        Command::Wallet { kind } => match kind {
            WalletCommand::Create => {
                ...
                println!(
                    "wallet={} mnemonic={:?} public_key={}",
                    out.wallet, out.mnemonic, out.public_key_hex
                );
            }
            WalletCommand::Balance { address } => {
                println!(
                    "{}",
                    balance_query(&address).unwrap_or_else(|err| {
        ...
        Command::Transfer {
            ...
            println!(
                "boc={:?}{}",
                boc,
                rpc.map(|r| format!(" rpc={r}")).unwrap_or_default()
            );
        }
